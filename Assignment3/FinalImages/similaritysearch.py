...
#INPUT FILE
 ...
 if user_input :
    ...
    next = user_input.rsplit('https://github.com/vanitapandit/Algorthmic-Digital-Marketing/blob/master/image/', 1)[1]
    
    final= next.rsplit('?raw=true', 1)[0]
...
#INPUT FILE
 ...
 if user_input :
    ...
    st.write("-------------------------------------------------------------------------------------------------")
    def get_data():
        return pd.read_csv('FAISSGit.csv')
    n=1
    df = get_data()
    images = df['0'].unique()
    #st.subheader("Select an image from the dropdown menu :point_down:")
